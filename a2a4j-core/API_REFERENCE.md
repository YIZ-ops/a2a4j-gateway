...
public class HttpCardResolver implements CardResolver {
    ...
    public AgentCard resolveCard() {
        return new HttpCardResolver("https://target-agent.example.com").resolveCard();
    }
}
```
...
A2AClient client = new DefaultA2AClient(new HttpCardResolver("https://target-agent.example.com"));
...
SendMessageResponse task = client.sendMessage(params);

// Stream messages
Flux<SendStreamingMessageResponse> stream = client.sendMessageStream(params);
stream.subscribe(
    event -> {
